...
router.get("/view/:id", async function (req, res) {
  ...
  if (!sessionId) {
    ...
    const list = await db.load(
      "select count(*) as count from customer_product where customerID = " +
        sessionId
    );
...
router.post("/view/:id", async function (req, res) {
  ...
  if (!customerID) {
    ...
    const product = await db.load(
      "SELECT products.productID, products.productName, products.productPrice, customer_product.size, customer_product.quantity, (products.productPrice*customer_product.quantity) as total FROM products, customer_product WHERE customer_product.productId = products.productID AND customer_product.customerID = " +
        customerID
    );
    var check = true;
    for (var i = 0; i < product.length; i++) {
      if (id == product[i].productID) {
        if (size == product[i].size) {
          ...
          await db.load(
            "update customer_product set quantity = " +
              quantity +
              " where productId = " +
              id +
              " and customerID = " +
