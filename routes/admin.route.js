...
router.get("/", async function (req, res) {
  ...
  var checked_paymentMethod = "0";
  if (!paymentMethod) {
    paymentMethod = "";
  } else if (paymentMethod === "cash") {
    checked_paymentMethod = 1;
  } else if (paymentMethod === "bank-transfer") {
    checked_paymentMethod = 2;
  }
  var rows;
  ...
  var sqlQuery = "SELECT * FROM orders WHERE status LIKE '%" + status + "%'";
  if (email) {
    sqlQuery += " AND email LIKE '%" + email + "%'";
  }

  if (paymentMethod) {
    sqlQuery += " AND paymentMethod = '" + paymentMethod + "'";
  }

  if (!day || day == "all") {
    day = "all";
    sqlQuery += " ORDER BY orderDate ASC";
  } else if (day == "today") {
    sqlQuery +=
      " AND CAST(orderDate AS DATE) = CURDATE() ORDER BY orderDate ASC";
  } else if (day == "7day") {
    sqlQuery +=
      " AND orderDate >= DATE_ADD(CURDATE(), INTERVAL -7 DAY) ORDER BY orderDate ASC";
  }

  rows = await db.load(sqlQuery);
...
router.get("/vieworder/:id", async function (req, res) {
  ...
  for (var i = 0; i < order_list.length; i++) {
    const stt = await db.load(
      "select * from productdetails where productID = " +
        order_list[i].productID +
        " and size = " +
        order_list[i].size
    );
  ...
  if (stt == 1) {
    ...
    if (check[0].status != status) {
      ...
      for (var i = 0; i < rows.length; i++) {
        await db.load(
          "update productdetails set quantityInStock = quantityInStock - " +
            rows[i].quantity +
            " where productID = " +
            rows[i].productID +
            " and size = " +
      ...
      const rows_ = await db.load(
        "select * from productdetails where quantityInStock <= 0"
      );
      await db.load("SET FOREIGN_KEY_CHECKS = 0");
      for (var i = 0; i < rows_.length; i++) {
        await db.load(
          "delete from customer_product where productId = " +
            rows_[i].productID +
            " and size = " +
            rows_[i].size
        );
...
router.get("/vieworder/:id/search", async function (req, res) {
  ...
  if (!isNaN(parseFloat(q)) && !isNaN(q - 0)) {
    rows = await db.load(
      "select p.*, pm.main, " +
        id +
        " as orderID from products p inner join productimages pm on p.productID = pm.productID where p.status ='SELLING' and p.productID = " +
        q
    );
  } else {
    rows = await db.load(
      "select p.*, pm.main, " +
        id +
        " as orderID from products p inner join productimages pm on p.productID = pm.productID where p.status = 'SELLING' and p.productName LIKE '%" +
        q +
        "%'"
    );
  }
...
router.get("/vieworder/:id/product/:productID", async function (req, res) {
  ...
  const product = await db.load(
    "SELECT * FROM products p, productimages pm WHERE p.productID = pm.productID and p.status = 'SELLING' and p.productID = " +
      pID
  );
  const productDetail = await db.load(
    "select * from productdetails where productID = + " +
      pID +
...
router.post("/vieworder/:id/product/:productID", async function (req, res) {
  ...
  const productInOrder = await db.load(
    "select * from orderdetails where orderNumber = " + id
  );
  var check = true;
...
router.post("/addNewProd", async function (req, res) {
  ...
  await db.load("SET FOREIGN_KEY_CHECKS = 0");
  await db.load(
    "insert into products (productID, productName, productBrand, productDescription, productPrice, style) values (" +
      id +
      ",'" +
      name +
      "', '" +
...
router.get("/editProduct", async function (req, res) {
  ...
  var list = await db.load(
    "select p.*, pm.main from products p inner join productimages pm on p.productID = pm.productID where p.status = 'SELLING'"
  );
  ...
  list = await db.load(
    "select p.*, pm.main from products p inner join productimages pm on p.productID = pm.productID where p.status = 'SELLING' limit 12 offset " +
      offset
  );
  res.render("adminEdit", {
    layout: "admin.main.handlebars",
...
router.get("/editProduct/:id", async function (req, res) {
  const id = req.params.id;
  const rows = await db.load(
    "select * from products where status ='SELLING' and productID = " + id
  );
  ...
  const size_ = [];
  const rows_size = await db.load(
    "select size, quantityInStock from productdetails where productID = " + id
  );
  for (var j = 37; j <= 45; j++) {
    var check = true;
...
router.get("/editProduct/:id/imageEdit", async function (req, res) {
  ...
  const list = await db.load(
    "select pm.* from productimages pm where productID = " + id
  );

  fs.mkdir(path.join("public", "img", id), { recursive: true }, (err) => {
    if (err) {
...
var multiUpload = upload.fields([
  ...
  { name: "view1", maxCount: 10 },
  { name: "view2", maxCount: 10 },
  { name: "view3", maxCount: 10 },
  { name: "view4", maxCount: 10 },
]);
router.post(
  "/editProduct/:id/imageEdit",
  multiUpload,
...
router.get("/delproduct/:id", async function (req, res) {
  ...
  await db.load(
    "update products set status = 'DELETE' where productID = " + id
  );
  await db.load("SET FOREIGN_KEY_CHECKS = 1");
...
router.get("/searchbyproduct", async function (req, res) {
  ...
  if (!isNaN(parseFloat(q)) && !isNaN(q - 0)) {
    rows = await db.load(
      "select p.*, pm.main from products p inner join productimages pm on p.productID = pm.productID where p.status ='SELLING' and p.productID = " +
        q
    );
  } else {
    rows = await db.load(
      "select p.*, pm.main from products p inner join productimages pm on p.productID = pm.productID where p.status = 'SELLING' and p.productName LIKE '%" +
        q +
        "%'"
    );
  }
